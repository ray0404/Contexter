...
def process_path_for_md(path, outfile, exclude_patterns, processed_files, matcher=None):
   ...
   if os.path.isdir(norm_path):
       ...
       if is_top_level:
           tree = generate_file_tree(norm_path, exclude_patterns, matcher=matcher)
       ...
       for root, dirs, files in os.walk(norm_path, topdown=True):
           ...
           for filename in files:
               ...
               process_path_for_md(file_path, outfile, exclude_patterns, processed_files, matcher=matcher)
   ...
   with open(args.output_file, 'w', encoding='utf-8') as outfile:
       for path in args.paths:
           ...
           if not matcher(os.path.basename(norm_path)):
               process_path_for_md(norm_path, outfile, exclude_patterns, processed_files, matcher=matcher)
