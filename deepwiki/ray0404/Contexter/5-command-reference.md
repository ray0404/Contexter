Loading...

![svg image](data:image/svg+xml;base64,)Index your code with Devin

[DeepWiki](/)

[DeepWiki](/)

[ray0404/Contexter ![svg image](data:image/svg+xml;base64,)](https://github.com/ray0404/Contexter "Open repository")

Index your code with

![svg image](data:image/svg+xml;base64,)Devin

![svg image](data:image/svg+xml;base64,)Edit Wiki![svg image](data:image/svg+xml;base64,)Share

Loading...

Last indexed: 2 November 2025 ([79ebf7](https://github.com/ray0404/Contexter/commits/79ebf745))

- [Overview](/ray0404/Contexter/1-overview)
- [Getting Started](/ray0404/Contexter/2-getting-started)
- [Installation](/ray0404/Contexter/2.1-installation)
- [Quick Start Guide](/ray0404/Contexter/2.2-quick-start-guide)
- [Command Overview](/ray0404/Contexter/2.3-command-overview)
- [Core Concepts](/ray0404/Contexter/3-core-concepts)
- [Context Files](/ray0404/Contexter/3.1-context-files)
- [Packaging and Reconstruction](/ray0404/Contexter/3.2-packaging-and-reconstruction)
- [Update Management](/ray0404/Contexter/3.3-update-management)
- [Exclusion Patterns](/ray0404/Contexter/3.4-exclusion-patterns)
- [Workflows](/ray0404/Contexter/4-workflows)
- [Basic Packaging Workflow](/ray0404/Contexter/4.1-basic-packaging-workflow)
- [AI Integration Workflow](/ray0404/Contexter/4.2-ai-integration-workflow)
- [Version Control with Patches](/ray0404/Contexter/4.3-version-control-with-patches)
- [Format Conversion Workflow](/ray0404/Contexter/4.4-format-conversion-workflow)
- [Command Reference](/ray0404/Contexter/5-command-reference)
- [Building Commands](/ray0404/Contexter/5.1-building-commands)
- [buildcontext](/ray0404/Contexter/5.1.1-buildcontext)
- [buildcontexthtml](/ray0404/Contexter/5.1.2-buildcontexthtml)
- [Reconstruction Commands](/ray0404/Contexter/5.2-reconstruction-commands)
- [reconstructor](/ray0404/Contexter/5.2.1-reconstructor)
- [reconstructorhtml](/ray0404/Contexter/5.2.2-reconstructorhtml)
- [Update Commands](/ray0404/Contexter/5.3-update-commands)
- [updatecontext](/ray0404/Contexter/5.3.1-updatecontext)
- [updater](/ray0404/Contexter/5.3.2-updater)
- [smartupdate](/ray0404/Contexter/5.3.3-smartupdate)
- [updatecontexthtml](/ray0404/Contexter/5.3.4-updatecontexthtml)
- [updaterhtml](/ray0404/Contexter/5.3.5-updaterhtml)
- [Utility Commands](/ray0404/Contexter/5.4-utility-commands)
- [sanitizecontext](/ray0404/Contexter/5.4.1-sanitizecontext)
- [md2html](/ray0404/Contexter/5.4.2-md2html)
- [html2md](/ray0404/Contexter/5.4.3-html2md)
- [Architecture](/ray0404/Contexter/6-architecture)
- [System Architecture](/ray0404/Contexter/6.1-system-architecture)
- [Module Organization](/ray0404/Contexter/6.2-module-organization)
- [Utility Layer (contexter\_utils)](/ray0404/Contexter/6.3-utility-layer-(contexter_utils))
- [Dependencies](/ray0404/Contexter/6.4-dependencies)
- [Technical Details](/ray0404/Contexter/7-technical-details)
- [Markdown Context Format](/ray0404/Contexter/7.1-markdown-context-format)
- [HTML Context Format](/ray0404/Contexter/7.2-html-context-format)
- [Patch File Format](/ray0404/Contexter/7.3-patch-file-format)
- [Binary File Handling](/ray0404/Contexter/7.4-binary-file-handling)

![svg image](data:image/svg+xml;base64,)Menu

# Command Referencesvg image

Relevant source files

- [![svg image](data:image/svg+xml;base64,)README.md](https://github.com/ray0404/Contexter/blob/79ebf745/README.md)
- [![svg image](data:image/svg+xml;base64,)pyproject.toml](https://github.com/ray0404/Contexter/blob/79ebf745/pyproject.toml)
- [![svg image](data:image/svg+xml;base64,)setup.py](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py)

This page provides comprehensive documentation for all command-line interface (CLI) tools included in the Contexter suite. The Contexter system exposes 12 distinct commands organized into four functional categories: building, reconstruction, update management, and utilities.

This page presents an overview of all commands, their organization, common patterns, and how they map to underlying implementation modules. For detailed documentation of individual commands including all options, arguments, and usage examples, refer to the subsection pages:

- Building commands: See [Building Commands](/ray0404/Contexter/5.1-building-commands)
- Reconstruction commands: See [Reconstruction Commands](/ray0404/Contexter/5.2-reconstruction-commands)
- Update/patch commands: See [Update Commands](/ray0404/Contexter/5.3-update-commands)
- Utility commands: See [Utility Commands](/ray0404/Contexter/5.4-utility-commands)

For information about workflows that combine multiple commands, see [Workflows](/ray0404/Contexter/4-workflows).

---

## Command Organizationsvg image

The Contexter CLI provides 12 commands that follow a consistent organizational pattern. Commands are grouped by function and follow a naming convention that distinguishes Markdown operations from HTML operations.

### Command Category Breakdownsvg image

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md20-36](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L20-L36) [![svg image](data:image/svg+xml;base64,)setup.py39-54](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L39-L54)

---

## Quick Reference Tablesvg image

| Command | Module | Category | Formats | Primary Function |
| --- | --- | --- | --- | --- |
| `buildcontext` | `context_builder` | Building | MD | Packages directory → Markdown context file |
| `buildcontexthtml` | `build_context_html` | Building | HTML | Packages directory → HTML context file |
| `reconstructor` | `reconstructor` | Reconstruction | MD | Rebuilds directory from Markdown context |
| `reconstructorhtml` | `reconstructor_html` | Reconstruction | HTML | Rebuilds directory from HTML context |
| `updatecontext` | `update_context` | Update | MD | Generates patch from two MD contexts (standard diff) |
| `updater` | `updater` | Update | MD | Applies patch to MD context |
| `smartupdate` | `smart_update` | Update | MD | Generates patch using rsync algorithm (recommended) |
| `updatecontexthtml` | `update_context_html` | Update | HTML | Generates patch from two HTML contexts |
| `updaterhtml` | `updater_html` | Update | HTML | Applies patch to HTML context |
| `sanitizecontext` | `sanitize_context` | Utility | MD | Fixes AI-generated Markdown formatting |
| `md2html` | `md2html` | Utility | MD→HTML | Converts Markdown file to HTML |
| `html2md` | `html2md` | Utility | HTML→MD | Converts HTML file to Markdown |

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md22-36](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L22-L36) [![svg image](data:image/svg+xml;base64,)setup.py40-52](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L40-L52)

---

## Command Categoriessvg image

### Building Commandssvg image

Commands that create context files from existing project directories. These commands scan a directory structure, aggregate file contents, and generate a single portable context file.

| Command | Output Format | Key Features |
| --- | --- | --- |
| `buildcontext` | Markdown | Syntax highlighting, exclusion patterns, binary detection |
| `buildcontexthtml` | HTML | Syntax highlighting, exclusion patterns, binary detection |

Both commands support identical functionality but differ in output format. See [Building Commands](/ray0404/Contexter/5.1-building-commands) for detailed documentation.

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md24-30](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L24-L30)

---

### Reconstruction Commandssvg image

Commands that rebuild project directory structures from context files. These commands parse context files, extract file paths and contents, recreate directory hierarchies, and write individual files.

| Command | Input Format | Key Features |
| --- | --- | --- |
| `reconstructor` | Markdown | Binary file restoration, directory structure preservation |
| `reconstructorhtml` | HTML | Binary file restoration, directory structure preservation |

Both commands produce identical directory structures regardless of input format. See [Reconstruction Commands](/ray0404/Contexter/5.2-reconstruction-commands) for detailed documentation.

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md25-31](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L25-L31)

---

### Update Commandssvg image

Commands that manage versioning and incremental updates through patch generation and application. This category includes five commands supporting both standard diff-based updates and high-performance rsync-based updates.

| Command | Operation | Formats | Algorithm |
| --- | --- | --- | --- |
| `updatecontext` | Patch generation | MD | Standard unified diff |
| `smartupdate` | Patch generation | MD | rsync delta encoding (recommended) |
| `updatecontexthtml` | Patch generation | HTML | Standard unified diff |
| `updater` | Patch application | MD | Standard patch |
| `updaterhtml` | Patch application | HTML | Standard patch |

The `smartupdate` command is recommended for better performance on large projects. See [Update Commands](/ray0404/Contexter/5.3-update-commands) for detailed documentation.

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md27-33](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L27-L33)

---

### Utility Commandssvg image

Specialized commands for AI workflow integration and format conversion.

| Command | Purpose | Input → Output |
| --- | --- | --- |
| `sanitizecontext` | Fix AI-generated code formatting | Raw AI output MD → Valid context MD |
| `md2html` | Format conversion | MD file → HTML file |
| `html2md` | Format conversion | HTML file → MD file |

The `sanitizecontext` command is new in v2.0.0 and addresses common formatting issues when AI models generate code without proper Markdown fences. See [Utility Commands](/ray0404/Contexter/5.4-utility-commands) for detailed documentation.

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md26-35](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L26-L35)

---

## CLI Entry Point Mappingsvg image

The following diagram shows how CLI commands map to Python implementation modules through setuptools entry points:

Each CLI command has a 1:1 mapping to a Python module containing a `main()` function that serves as the entry point. This mapping is defined in [![svg image](data:image/svg+xml;base64,)setup.py39-54](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L39-L54) within the `entry_points` dictionary under `console_scripts`.

**Sources:** [![svg image](data:image/svg+xml;base64,)setup.py39-54](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L39-L54)

---

## Common Patterns and Conventionssvg image

### Command Naming Conventionsvg image

Commands follow a consistent naming pattern:

- **Base commands**: Use lowercase single words (e.g., `reconstructor`, `updater`)
- **Format-specific variants**: Append format indicator to base command (e.g., `buildcontexthtml`, `reconstructorhtml`)
- **Compound operations**: Use concatenated words (e.g., `updatecontext`, `sanitizecontext`)
- **Format conversion**: Use format abbreviations with direction (e.g., `md2html`, `html2md`)

### Common Command Optionssvg image

While each command has specific options (documented in subsections), all commands support:

- `--help` or `-h`: Display help message with all options and usage examples
- Positional arguments: Most commands use positional arguments for required inputs/outputs

### Output Conventionssvg image

| Command Type | Default Output Behavior |
| --- | --- |
| Building | Writes to specified output file path |
| Reconstruction | Creates/overwrites directory at specified path |
| Update (generation) | Writes patch to specified file path |
| Update (application) | Writes updated context to specified file path |
| Utility | Writes converted/sanitized file to specified path |

---

## Getting Helpsvg image

To see detailed usage information for any command, run it with the `--help` flag:

Each command's help output includes:

- Description of the command's purpose
- Required positional arguments
- Optional flags and their effects
- Usage examples
- Related commands

**Example help output structure:**

```px-2
usage: buildcontext [-h] [options] source_dir output_file

Package a directory into a Markdown context file.

positional arguments:
  source_dir    Path to the directory to package
  output_file   Path where the context file will be written

optional arguments:
  -h, --help    Show this help message and exit
  ...
```

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md37](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L37-L37)

---

## Installation and Requirementssvg image

All commands are installed simultaneously when installing the Contexter package. The commands are registered as console scripts through setuptools and become available system-wide (or within the virtual environment) after installation.

### Required Dependenciessvg image

Commands depend on the following external libraries:

| Library | Purpose | Used By |
| --- | --- | --- |
| `markdown` | Markdown parsing and rendering | Building, conversion commands |
| `Pygments` | Syntax highlighting | Building commands |
| `beautifulsoup4` | HTML parsing | HTML commands, conversion |
| `markdownify` | HTML to Markdown conversion | `html2md` |
| `patch` | Patch generation and application | Update commands |

These dependencies are automatically installed when installing Contexter via pip. See [Dependencies](/ray0404/Contexter/6.4-dependencies) for detailed information about dependency usage.

**Sources:** [![svg image](data:image/svg+xml;base64,)pyproject.toml23-29](https://github.com/ray0404/Contexter/blob/79ebf745/pyproject.toml#L23-L29) [![svg image](data:image/svg+xml;base64,)setup.py11-12](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L11-L12)

---

## Module Architecture Overviewsvg image

The following diagram illustrates how commands, modules, and shared utilities interact:

All implementation modules leverage shared functionality from `contexter_utils.py`, which centralizes common operations like file processing, parsing, and reconstruction logic. For details on the utility layer, see [Utility Layer](/ray0404/Contexter/6.3-utility-layer-(contexter_utils)).

**Sources:** [![svg image](data:image/svg+xml;base64,)setup.py23-36](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L23-L36) [![svg image](data:image/svg+xml;base64,)setup.py39-54](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L39-L54)

---

## Command Relationshipssvg image

Commands are designed to work together in workflows. The following table shows common command combinations:

| Workflow | Commands | Purpose |
| --- | --- | --- |
| **Full Build-Reconstruct Cycle** | `buildcontext` → `reconstructor` | Package project, then rebuild elsewhere |
| **AI Code Generation** | AI output → `sanitizecontext` → `reconstructor` | Convert AI-generated code to working project |
| **Version Management** | `buildcontext` (v1) → `buildcontext` (v2) → `smartupdate` → `updater` | Track changes between versions |
| **Format Migration** | `buildcontext` → `md2html` or `buildcontexthtml` → `html2md` | Switch between MD and HTML formats |
| **Cross-Format Update** | `buildcontext` → `updatecontext` → `updaterhtml` | Apply MD patch to HTML context |
| **HTML Workflow** | `buildcontexthtml` → `reconstructorhtml` | Complete HTML-based cycle |

For detailed workflow examples, see [Workflows](/ray0404/Contexter/4-workflows).

**Sources:** [![svg image](data:image/svg+xml;base64,)README.md15-18](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L15-L18) [![svg image](data:image/svg+xml;base64,)README.md22-36](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L22-L36)

---

## Version Informationsvg image

Commands follow the Contexter package version number. The current version is defined in both `setup.py` and `pyproject.toml`:

- **Version 2.1.0** in [![svg image](data:image/svg+xml;base64,)setup.py16](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L16-L16)
- **Version 2.0.0** in [![svg image](data:image/svg+xml;base64,)pyproject.toml9](https://github.com/ray0404/Contexter/blob/79ebf745/pyproject.toml#L9-L9)

Version 2.0.0 introduced the `sanitizecontext` command for AI workflow integration. Check the version-specific release notes for details on command additions and modifications.

**Sources:** [![svg image](data:image/svg+xml;base64,)setup.py16](https://github.com/ray0404/Contexter/blob/79ebf745/setup.py#L16-L16) [![svg image](data:image/svg+xml;base64,)pyproject.toml9](https://github.com/ray0404/Contexter/blob/79ebf745/pyproject.toml#L9-L9) [![svg image](data:image/svg+xml;base64,)README.md9-18](https://github.com/ray0404/Contexter/blob/79ebf745/README.md#L9-L18)

---

## Next Stepssvg image

For detailed information about specific commands:

- **Building operations**: See [buildcontext](/ray0404/Contexter/5.1.1-buildcontext) and [buildcontexthtml](/ray0404/Contexter/5.1.2-buildcontexthtml)
- **Reconstruction operations**: See [reconstructor](/ray0404/Contexter/5.2.1-reconstructor) and [reconstructorhtml](/ray0404/Contexter/5.2.2-reconstructorhtml)
- **Update operations**: See [updatecontext](/ray0404/Contexter/5.3.1-updatecontext), [updater](/ray0404/Contexter/5.3.2-updater), [smartupdate](/ray0404/Contexter/5.3.3-smartupdate), [updatecontexthtml](/ray0404/Contexter/5.3.4-updatecontexthtml), and [updaterhtml](/ray0404/Contexter/5.3.5-updaterhtml)
- **Utility operations**: See [sanitizecontext](/ray0404/Contexter/5.4.1-sanitizecontext), [md2html](/ray0404/Contexter/5.4.2-md2html), and [html2md](/ray0404/Contexter/5.4.3-html2md)

For conceptual background on context files and workflows, see [Core Concepts](/ray0404/Contexter/3-core-concepts).

![svg image](data:image/svg+xml;base64,)Dismiss

Refresh this wiki

Enter email to refresh

### On this page

- [Command Reference](#command-reference)
- [Command Organization](#command-organization)
- [Command Category Breakdown](#command-category-breakdown)
- [Quick Reference Table](#quick-reference-table)
- [Command Categories](#command-categories)
- [Building Commands](#building-commands)
- [Reconstruction Commands](#reconstruction-commands)
- [Update Commands](#update-commands)
- [Utility Commands](#utility-commands)
- [CLI Entry Point Mapping](#cli-entry-point-mapping)
- [Common Patterns and Conventions](#common-patterns-and-conventions)
- [Command Naming Convention](#command-naming-convention)
- [Common Command Options](#common-command-options)
- [Output Conventions](#output-conventions)
- [Getting Help](#getting-help)
- [Installation and Requirements](#installation-and-requirements)
- [Required Dependencies](#required-dependencies)
- [Module Architecture Overview](#module-architecture-overview)
- [Command Relationships](#command-relationships)
- [Version Information](#version-information)
- [Next Steps](#next-steps)
